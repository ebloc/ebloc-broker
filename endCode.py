...
def endCall(jobKey, index, cloudStorageID, shareToken, folderName, slurmJobID):
    ...
    log('To run again:')
    log('~/eBlocBroker/endCode.py ' + jobKey + ' ' + index + ' ' + cloudStorageID + ' ' + shareToken + ' ' + folderName + ' ' + slurmJobID + '\n')
    ...
    while True:
        ...
        count += 1
        if jobInfo['jobStateCode'] == lib.job_state_code['RUNNING']: # It will come here eventually, when setJob() is deployed.
