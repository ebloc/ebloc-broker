...
def check_before_submit(self, provider, _from, provider_info, key, job):
    ...
    block_read_from, orcid = self.eBlocBroker.functions.getRequesterInfo(_from).call()
    ...
    if len(key) != 46 and main_storageID in (StorageID.IPFS, StorageID.IPFS_MINILOCK):
        logging.error(
            "E: key's length does not match with its original length, it should be 46. Please check your key length"
        )
        raise

    if len(key) != 33 and StorageID.GDRIVE == main_storageID:
        logging.error(
    ...
    for idx, core in enumerate(job.cores):
        ...
        if job.execution_durations[idx] == 0:
            logging.error(f"E: execution_durations[{idx}] is provided as 0. Please provide non-zero value")
            raise
    ...
    for core_min in job.execution_durations:
        if core_min > 1440:
            logging.error("E: execution_durations provided greater than 1440. Please provide smaller value")
            raise
