...
from lib import convertByteToMB, silentremove, log, sbatchCall
    ...
    if cacheType == lib.CacheType.PRIVATE.value:  # First checking does is already exist under public cache directory
        ...
        if folderType == "gzip":
            ...
            if os.path.isfile(globalCachePath + "/" + folderName + "/" + sourceCodeHash + ".tar.gz"):
                ...
                res = (
                    subprocess.check_output(["bash", lib.EBLOCPATH + "/scripts/generateMD5sum.sh", tarFile])
                    .decode("utf-8")
                    .strip()
                )
            else:
                res = (
                    subprocess.check_output(
                        ["bash", lib.EBLOCPATH + "/scripts/generateMD5sum.sh", globalCachePath + "/" + folderName]
                    )
                    .decode("utf-8")
                    .strip()
                )
...
def gdriveDownloadFolder(resultsFolderPrev, folderName, folderType, key) -> bool:
    if folderType == "folder":
        ...
        status, res = lib.subprocessCallAttempt(
            ["gdrive", "download", "--recursive", key, "--force", "--path", resultsFolderPrev], 10
        )
        if not status:
            return False
...
def getData(key, requester, resultsFolderPrev, resultsFolder, jobKeyFlag=False):
    ...
    if jobKeyFlag:
        ...
        status, used_dataTransferIn, globals()["jobKey_list"], key = lib.gdriveSize(
            key, mimeType, folderName, gdriveInfo, resultsFolderPrev, sourceCodeHash_list, shouldAlreadyCached
        )
        if not status:
            return False
    ...
    if cacheType == lib.CacheType.PRIVATE.value or cacheType == lib.CacheType.PUBLIC.value:
        if "gzip" in mimeType:  # Recieved job is in folder tar.gz
            ...
            status, ipfsHash = cache(
                requester, resultsFolderPrev, folderName, sourceCodeHash, "folder", key, jobKeyFlag
            )
            if not status:
                return False
            ...
            if os.path.isfile(resultsFolder + "/" + folderName + ".tar.gz"):
                command = [
                    "tar",
                    "-xf",
                    resultsFolder + "/" + folderName + ".tar.gz",
                    "--strip-components=1",
                    "-C",
                    resultsFolder,
                ]
...
def driverGdrive(loggedJob, jobInfo, requester, shouldAlreadyCached, eBlocBroker, w3) -> bool:
    ...
    resultsFolderPrev = lib.PROGRAM_PATH + "/" + requester + "/" + jobKey + "_" + str(loggedJob.args["index"])
