...
if [ -z "$(ls -A $sourceFolder)" ]; then 
    ...
    rsync -av --progress $sourceFolder/ $toFolder --exclude venv --exclude 'nohup.out' --exclude node_modules --exclude='.git' --exclude .env --exclude /README.md --exclude contract --exclude .gitignore --filter="dir-merge,- .gitignore" --delete 
fi 
