...
index = None
cacheType = None
globalCacheFolder = None
sourceCodeHash_list = None
shouldAlreadyCached = None
jobID = None
shareID = {}
folderType_dict = {}
...
def cache(resultsFolderPrev, folderName, _id):
    global cacheType
    global globalCacheFolder
    global folderType_dict

    if (
        cacheType[_id] == lib.CacheType.PRIVATE.value
    ):  # First checking does is already exist under public cache directory
        globalCacheFolder = publicDir
        ...
        if not os.path.isfile(cachedTarFile):
            if os.path.isfile(cachedFolder + "/run.sh"):
                res = (
                    subprocess.check_output(["bash", lib.EBLOCPATH + "/scripts/generateMD5sum.sh", cachedFolder])
                    .decode("utf-8")
                    .strip()
                )
                if res == folderName:  # Checking is already downloaded folder's hash matches with the given hash
                    folderType_dict[folderName] = "folder"
                    cacheType[_id] = lib.CacheType.PUBLIC.value
                    log("Already cached under the public directory...", "green")
                    return True
        else:
            folderType_dict[folderName] = "tar.gz"
            res = (
                subprocess.check_output(["bash", lib.EBLOCPATH + "/scripts/generateMD5sum.sh", cachedTarFile])
                .decode("utf-8")
                .strip()
            )
            if res == folderName:  # Checking is already downloaded folder's hash matches with the given hash
    ...
    if cacheType[_id] == lib.CacheType.PRIVATE.value or cacheType[_id] == lib.CacheType.PUBLIC.value:
        if cacheType[_id] == lib.CacheType.PRIVATE.value:
            # Download into private directory at $HOME/.eBlocBroker/cache
            globalCacheFolder = privateDir
        elif cacheType[_id] == lib.CacheType.PUBLIC.value:
            globalCacheFolder = publicDir
            ...
            if os.path.isdir(cachedFolder):
                res = (
                    subprocess.check_output(
                        [
                            "bash",
                            lib.EBLOCPATH + "/scripts/generateMD5sum.sh",
                            cachedFolder + "/" + folderName + ".tar.gz",
                        ]
                    )
                    .decode("utf-8")
                    .strip()
...
def eudatDownloadFolder(resultsFolderPrev, resultsFolder, folderName) -> bool:
    ...
    for attempt in range(5):
        ...
        status, result = refund(
            PROVIDER_ID, PROVIDER_ID, jobKey, index, jobID, sourceCodeHash_list
...
def eudatGetShareToken():
    ...
    for i in range(0, len(sourceCodeHashText_list)):
        ...
        folderType_dict[folderName] = None  # Initialization
        if os.path.isdir(lib.OWN_CLOUD_PATH + "/" + folderName):
            log(
                "Eudat shared folder(" + folderName + ") is already accepted and exists on the Eudat mounted folder...",
                "green",
            )
        ...
        if folderTokenFlag[folderName]:
            ...
            for i in range(len(shareList) - 1, -1, -1):  # Starts iterating from last item to the first one
                ...
                if inputFolderName == folderName and inputUser == fID:
                    ...
                    shareID[folderName] = {"shareID": int(_shareID), "shareToken": shareToken}
    ...
    with open(shareID_file, "w") as f:
        json.dump(shareID, f)  # TODO: dump into mongodb
...
def driverEudat(loggedJob, jobInfo, requesterID, _shouldAlreadyCached, eBlocBroker, w3, _oc) -> bool:
    status, providerInfo = get_provider_info(loggedJob.args["provider"])
    global sourceCodeHashText_list
    global fID
    global jobKey
    global oc
    global privateDir
    ...
    global index
    global cacheType
    global sourceCodeHash_list
    global shouldAlreadyCached
    global jobID

    fID = providerInfo["fID"]
    jobKey = loggedJob.args["jobKey"]
    oc = _oc
    privateDir = PROGRAM_PATH + "/" + requesterID + "/cache"
    publicDir = PROGRAM_PATH + "/cache"
    index = loggedJob.args["index"]
    cacheType = loggedJob.args["cacheType"]
    sourceCodeHash_list = loggedJob.args["sourceCodeHash"]
    shouldAlreadyCached = _shouldAlreadyCached

    dataTransferIn = jobInfo[0]["dataTransferIn"]
    jobID = 0
        ...
        if folderType_dict[folderName] == "tar.gz":
            ...
            res = subprocess.run(
                [
                    "tar",
                    "-xf",
                    globalCacheFolder + "/" + folderName + ".tar.gz",
                    "--strip-components=1",
                    "-C",
                    resultsFolder,
                ]
