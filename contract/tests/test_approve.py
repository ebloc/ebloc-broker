...
def test_initial_approval_is_zero(_Ebb, accounts, idx):
    assert _cfg.TOKEN.allowance(accounts[0], accounts[idx]) == 0


def test_approve(_Ebb, accounts):
    _cfg.TOKEN.approve(accounts[1], 10**2, {"from": accounts[0]})

    assert _cfg.TOKEN.allowance(accounts[0], accounts[1]) == 10**2


def test_modify_approve(_Ebb, accounts):
    _cfg.TOKEN.approve(accounts[1], 10**2, {"from": accounts[0]})
    _cfg.TOKEN.approve(accounts[1], 12345678, {"from": accounts[0]})

    assert _cfg.TOKEN.allowance(accounts[0], accounts[1]) == 12345678
...
def test_revoke_approve(_Ebb, accounts):
    ...
    assert _cfg.TOKEN.allowance(accounts[0], accounts[1]) == 0


def test_approve_self(_Ebb, accounts):
    _cfg.TOKEN.approve(accounts[0], 10**2, {"from": accounts[0]})

    assert _cfg.TOKEN.allowance(accounts[0], accounts[0]) == 10**2


def test_only_affects_target(_Ebb, accounts):
    _cfg.TOKEN.approve(accounts[1], 10**2, {"from": accounts[0]})

    assert _cfg.TOKEN.allowance(accounts[1], accounts[0]) == 0


def test_returns_true(_Ebb, accounts):
    tx = _cfg.TOKEN.approve(accounts[1], 10**2, {"from": accounts[0]})
