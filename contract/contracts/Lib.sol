...
library Lib {
            ...
            if (_interval.startTime >= currentNode_endpoint) {
                if (_interval.startTime == currentNode_endpoint) {
                    ...
                    if (temp == 0) {
                        ...
                        if (temp > _interval.availableCore)
                            return false;
    ...
    function checkIfOverlapExists(IntervalNode storage self, IntervalArg memory _interval) internal returns (uint256 flag) {
        // LL storage ll = self.ll;
        uint256 addr = self.ll.tail;
        uint256 addrTemp;
        uint256 prevNode_index;
        uint256 currentNode_index;
        ...
        if (_interval.completionTime <= self.ll.items[addr].endpoint) {
        ...
        uint256 _length = self.ll.length;
        if (flag <= 1) {
            /* Inserted while keeping sorted order */
            push(self.ll, addr, _interval.completionTime, _interval.core, uint32(_length));
            _length += 1;
            carriedSum = _interval.core;

            if (flag == 0) {
                addrTemp = addr;
                prevNode_index = self.ll.tail = uint32(_length - 1);
            } else {
                addrTemp = self.ll.items[prevNode_index].next;
                self.ll.items[prevNode_index].next = uint32(_length - 1);
        ...
        currentNode_index = addrTemp; //self.ll.items[prevNode_index].next;
        if (
            _iterate(
                self.ll,
                currentNode_index,
            ...
            if (flag == 2)
                self.ll.items[addr].core = updatedCoreVal;

            return 1; // true
        } else {
            if (flag <= 1) {
                delete self.ll.items[uint32(_length - 1)];

                if (prevNode_index == self.ll.tail) {
                    self.ll.tail = uint32(addrTemp);
                } else {
                    self.ll.items[prevNode_index].next = uint32(addrTemp); // change on storage
                }
            } else if (flag == 3) {
                self.ll.items[prevNode_index].next = uint32(prevNode_index_next_temp);
            }
            return 0; // false
        }
    }
