...
library Lib {
    ...
    function _iterate(
        ...
        uint256 currentNode_endpoint = self.items[currentNodeIndex].endpoint; // read from the storage
        int32 currentNode_core = self.items[currentNodeIndex].core;
        uint32 currentNode_next = self.items[currentNodeIndex].next;
        do {
            if (_interval.startTimestamp >= currentNode_endpoint) {
                if (_interval.startTimestamp == currentNode_endpoint) {
                    int32 temp = currentNode_core + (_interval.core * -1);
                    if (temp == 0) {
                        self.items[prevNodeIndex].next = currentNode_next;
            ...
            currentNodeIndex = currentNode_next; // already in the memory, cheaper
            currentNode_endpoint = self.items[currentNodeIndex].endpoint; // read from the storage
            currentNode_core = self.items[currentNodeIndex].core;
            currentNode_next = self.items[currentNodeIndex].next;
        } while (true);
    }
    ...
    function _iterateStart(
        ...
        returns (
            ...
            uint256,
            int32 carriedSum,
            uint32 prevNodeIndex_next_temp,
            int32 updatedCoreVal
        )
    {
        flag = 1;
        uint32 currentNode_endpoint = self.items[prevNodeIndex].endpoint;
        uint32 currentNode_next = self.items[prevNodeIndex].next;
        int32 currentNode_core = self.items[prevNodeIndex].core;
        int32 temp;
        do {
            /* Inside while loop carriedSum is updated. If enters into if statement it
            means revert() is catched and all the previous operations are reverted back */
            if (_interval.endTimestamp >= currentNode_endpoint) {
                ...
                if (_interval.endTimestamp == currentNode_endpoint) {
                    ...
                    if (temp != 0) {
                        ...
                        prevNodeIndex_next_temp = self.items[prevNodeIndex].next;
                        self.items[currentNodeIndex] = self.items[currentNode_next];
                    }
                }
                _addr = currentNodeIndex; /* "addr" points the index to be added into the linked list */
                return (flag, _addr, prevNodeIndex, carriedSum, prevNodeIndex_next_temp, temp);
            }
            carriedSum += currentNode_core;
            if (carriedSum > _interval.availableCore) {
                return (0, _addr, 0, 0, 0, 0);
            ...
            prevNodeIndex = currentNodeIndex;
            currentNodeIndex = currentNode_next;
            currentNode_endpoint = self.items[currentNodeIndex].endpoint;
            currentNode_next = self.items[currentNodeIndex].next;
            currentNode_core = self.items[currentNodeIndex].core;
        } while (true);
    }
    ...
    function checkIfOverlapExists(LL storage self, IntervalArg memory _interval) internal returns (uint256 flag) {
        ...
        uint256 prevNodeIndex;
        uint256 currentNodeIndex;
        uint256 prevNodeIndex_next_temp;
        int32 carriedSum;
        int32 updatedCoreVal;
        if (_interval.endTimestamp <= self.items[addr].endpoint) {
            ...
            (flag, addr, prevNodeIndex, carriedSum, prevNodeIndex_next_temp, updatedCoreVal) = _iterateStart(
                self,
                prevNodeIndex,
                currentNodeIndex,
                _interval
        ...
        if (_iterate(self, currentNodeIndex, prevNodeIndex, uint32(_length), carriedSum, _interval)) {
            ...
            if (flag <= 1) {
                ...
                else self.items[prevNodeIndex].next = uint32(addrTemp); // change on storage
            } else if (flag == 3) {
                self.items[prevNodeIndex].next = uint32(prevNodeIndex_next_temp);
            }
            return 0; // false
        }
    }
