...
contract eBlocBroker is eBlocBrokerInterface, eBlocBrokerBase {
        ...
        whenProviderRunning
   {
        ...
        payable
    {
        ...
        (totalCost, dataTransferIn[0], storageCost, storageDuration[0]) = _calculateCacheCost(provider, args, sourceCodeHash, dataTransferIn, storageDuration, info);
        ...
        public
    {
        ...
        for (uint256 i = 0; i < sourceCodeHash.length; i++) {
            ...
            if (!jobSt.isVerifiedUsed) // False if already verified and used
        ...
        )
    {
        ...
        for (uint256 i = 0; i < sourceCodeHash.length; i++) {
            ...
            if (!(storageInfo.received > 0 || (jobSt.receivedBlock + jobSt.storageDuration >= block.number && !jobSt.isPrivate && jobSt.isVerifiedUsed))) {
                ...
                if (_temp == 0) { // If returned value of _checkRegisteredData is False move on to next condition
                    ...
                    emit LogRegisteredDataRequestToUse(args.provider, _sourceCodeHash);
            }
        }
