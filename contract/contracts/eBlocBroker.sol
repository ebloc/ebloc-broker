    ...
    EBlocBrokerBase
{
    ...
    */
    ...
    function submitJobMinimal1(address _provider, string memory key, uint32[] memory dataTransferIn, uint32  dataTransferOut
    ) public {
        Lib.Status storage jobInfo = providers[_provider].jobStatus[key][0];
        jobInfo.dataTransferIn = dataTransferIn[0];
        jobInfo.dataTransferOut = dataTransferOut;
        return;
    }

    function submitJobMinimal2(
        address _provider,
        string memory key,
        uint32[] memory dataTransferIn,
        uint16 runTime,
        ...
        uint256 cacheCost = info.priceCache.mul(dataTransferIn[0]);
        uint256 cost = uint256(info.priceCoreMin).mul(uint256(runTime));
        cost.add(cacheCost);
        cost.add(info.priceDataTransfer.mul(dataTransferIn[0].add(dataTransferOut)));
        Lib.Status storage jobInfo = providers[_provider].jobStatus[key][0];
        jobInfo.received = cost;
        jobInfo.cacheCost = cacheCost;
        return;
    }

    function submitJobMinimal3(
        address _provider,
        string memory key,
        uint32[] memory dataTransferIn,
        uint16 runTime,
        ...
        Lib.Status storage jobInfo = providers[_provider].jobStatus[key][0];
        Lib.Provider storage provider = providers[_provider];
        ERC20(tokenAddress).transferFrom(msg.sender, address(this), jobInfo.received);
        emit LogJob(
            _provider,
            msg.sender,
            key,
            ...
            runTime,
            jobInfo.received,
            jobPrice - jobInfo.received, // refunded
            0
        );
    }
