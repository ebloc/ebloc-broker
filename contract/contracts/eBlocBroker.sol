    ...
    EBlocBrokerBase
{
    ...
    */
    ...
    function submitJobMinimal(
        ...
        uint256 cost = uint256(info.priceCoreMin).mul(uint256(args.core[0]).mul(uint256(args.runTime[0])));
        cost.add(cacheCost);
        cost.add(info.priceDataTransfer.mul(dataTransferIn[0].add(args.dataTransferOut)));
        //
        ERC20(tokenAddress).transferFrom(msg.sender, address(this), cost);
        Lib.Status storage jobInfo = providers[args.provider].jobStatus[key][0];
        //
        jobInfo.cacheCost = cacheCost;
        jobInfo.dataTransferIn = dataTransferIn[0];
        jobInfo.dataTransferOut = args.dataTransferOut;
        // jobInfo.pricesSetBlockNum = 0;
