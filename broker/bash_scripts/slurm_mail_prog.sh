...
if [[ $c == *" Began, "* ]]; then
    ...
    if [ "$arg0" != "$arg1" ]; then  # job_key and index should not be same
        ...
        nohup python3 -uB $EBLOCBROKER_PATH/start_code.py \
              $arg0 $arg1 $slurm_job_id >/dev/null 2>&1
    fi
fi
...
if [[ $event == *"COMPLETED"* ]] || [[ $event == *"FAILED"* ]]; then
    ...
    if [ "$arg0" != "$arg1" ]; then  # job_key and index should not be same
        source $VENV_PATH/bin/activate
        nohup python3 -uB $EBLOCBROKER_PATH/end_code.py \
              $arg0 $arg1 $arg2 $name $slurm_job_id >/dev/null 2>&1
    fi
fi
...
if [[ $event == *"TIMEOUT"* ]]; then
    ...
    if [ "$arg0" != "$arg1" ]; then  # job_key and index should not be same
        source $VENV_PATH/bin/activate
        python3 -uB $EBLOCBROKER_PATH/end_code.py \
                $arg0 $arg1 $arg2 $name $slurm_job_id
    fi
fi
