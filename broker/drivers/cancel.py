...
def cancel_jobs(cancel_block_read_from_local):
    ...
    while True:
        ...
        logged_jobs_to_process = Ebb.LogJob.run_log_cancel_refund(
            cancel_block_read_from_local, env.PROVIDER_ID
        )
        for logged_job in logged_jobs_to_process:
            msg_sender = Ebb.w3.eth.get_transaction_receipt(
                logged_job["transactionHash"].hex()
            )["from"].lower()
            ...
            try:
                ...
                if job_id.isdigit():
                    ...
                    p2 = subprocess.Popen(
                        ["grep", "JobState="], stdin=p1.stdout, stdout=subprocess.PIPE
                    )
                    p1.stdout.close()
                    ...
                    if "JobState=CANCELLED" in out:
                        ...
                        log(
                            f"E: job_id={job_id} is not cancelled, something went wrong or already cancelled. {out}"
                        )
            except IndexError:
        ...
        if int(max_val) != 0:
            value = max_val + 1
            f_blockReadFrom = open(
                env.CANCEL_BLOCK_READ_FROM_FILE, "w"
