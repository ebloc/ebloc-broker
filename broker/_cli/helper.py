...
class Helper:
    ...
    def __init__(self):
        ...
        self.subparsers = self.parser.add_subparsers(
            dest="command", metavar="command [<options>...]"
        )
    ...
    def init(self):
        obj = self.subparsers.add_parser(
            "init", help="Initialize the ebloc-broker project"
        )
        obj.add_argument(
    ...
    def driver(self):
        obj = self.subparsers.add_parser(
            "driver",
            help="Driver program",
            epilog="run: nohup ebloc-broker > cmd.log &!",
        )
        obj.add_argument(
        ...
        obj.add_argument(
            "--latest",
            action="store_true",
            help="Block number to start fetch blocks from latest",
        ).completer = EnvironCompleter
        obj.add_argument(
    ...
    def run(self):
        ...
        """
        obj = self.subparsers.add_parser(
            "run", help="Run a script in the eblocbroker_scripts/ folder"
        )
        obj.add_argument(
            "--tx-receipt",
            type=str,
            metavar="[tx_hash]",
            help="Return Transaction Receipt",
        ).completer = EnvironCompleter
        obj.add_argument(
            "--authenticate-orcid",
            type=str,
    ...
    def submit(self):
        ...
        obj.add_argument(
            "path",
            type=str,
            help="Full file path of Yaml file that contains the job info",
        )
        # obj.add_argument('path', nargs='+', help='Full file path of Yaml file that contains the job info')

    def data(self):
        obj = self.subparsers.add_parser(
            "data", help="List registered data files of the provider"
        )
        obj.add_argument(
            "eth_address", type=str, help="Ethereum address of the provider"
        )

    def balance(self):
        # FIXME: return balance and convert it into usd
        obj = self.subparsers.add_parser(
            "balance", help="Returns user's earned money amount in USD token."
        )
        obj.add_argument(
            "eth_address", type=str, help="Ethereum address of the provider"
        )
